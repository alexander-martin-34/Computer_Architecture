...
void CPU::decode() {
  ...
  opcode = (instr >> 26) & 0x3f;//Shift register and mask with 0x3f
  rs = (instr >> 21) & 0x1f;//Shift register and mask with 0x1f etc.->
  rt = (instr >> 16) & 0x1f;
  rd = (instr >> 11) & 0x1f;
  shamt = (instr >> 6) & 0x1f; 
  funct = (instr & 0x3f); 
  uimm = (instr & 0xffff);
  simm = ((signed((instr & 0xffff)<<16)) >> 16);
